...
async function runScenario(viewport) {
  ...
  try {
    ...
    await page.touchscreen.tap(viewport.width - 108, viewport.height - 35);
    await delay(550);
...
async function run() {
  const results = [];
  results.push(await runScenario({ name: 'phone-portrait', width: 390, height: 844, cardX: 195, cardY: 319 }));
  results.push(await runScenario({ name: 'phone-landscape', width: 844, height: 390, cardX: 232, cardY: 295 }));
