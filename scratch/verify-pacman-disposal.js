...
async function run() {
  ...
  try {
    ...
    // Loop Pac-Man entry and exit twice to ensure no canvas accumulation
    for (let iteration = 1; iteration <= 2; iteration++) {
      console.log(`\n--- Iteration ${iteration} ---`);
      
      // 2. Launch Pacman Scene
      console.log('Scrolling and launching Pacman...');
      await page.evaluate(() => {
        const game = window.__WGF_GAME__;
      ...
      console.log(`Canvases in Pacman game: ${canvasesInGame}`);
      if (canvasesInGame !== 2) {
        throw new Error(`Expected 2 canvases in Pacman game, found ${canvasesInGame}`);
      }
      ...
      // 3. Return to Hub (Tap back button at top left: 20, 16)
